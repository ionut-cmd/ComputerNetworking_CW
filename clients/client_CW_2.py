...
def handle_server():
    ...
    while connected:
        ...
        # when server sends an invitation we save it
            ...
            print(f'TESTING FILE RECIEVED {data}')
            # file_data = " ".join(data[2:]).encode(FORMAT)
            file_data = get_file(server_data).encode(FORMAT)
            print(f'FILE_DATA===={file_data}')
            sending_user = " ".join(data[:1])
            print(f'SENDING_USER===={sending_user}')
        ...
        elif message == 'Response_User_Present':
